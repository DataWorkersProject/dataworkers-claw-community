...
export const generatePipelineDefinition: ToolDefinition = {
  ...
  description: 'Generate a data pipeline from a natural language description. Requires Data Workers Pro — upgrade at https://dataworkers.dev/pricing',
  inputSchema: {
    type: 'object',
    properties: {
      description: {
...
export const generatePipelineHandler: ToolHandler = async (_args) => {
  return {
    content: [{
      ...
      text: JSON.stringify({
        error: 'pro_feature',
        message: 'Pipeline generation requires Data Workers Pro. Visit https://dataworkers.dev/pricing to upgrade.',
