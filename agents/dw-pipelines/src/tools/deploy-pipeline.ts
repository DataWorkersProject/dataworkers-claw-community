...
export const deployPipelineDefinition: ToolDefinition = {
  ...
  description: 'Deploy a validated pipeline to the target orchestrator. Requires Data Workers Pro — upgrade at https://dataworkers.io/pricing',
  inputSchema: {
    type: 'object',
    properties: {
      pipelineSpec: {
...
export const deployPipelineHandler: ToolHandler = async (_args) => {
  return {
    content: [{
      ...
      text: JSON.stringify({
        error: 'pro_feature',
        message: 'Pipeline deployment requires Data Workers Pro. Visit https://dataworkers.io/pricing to upgrade.',
