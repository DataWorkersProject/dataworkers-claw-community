...
export const applyMigrationHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('apply_migration')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'apply_migration' }) }],
      isError: true,
    };
  }
