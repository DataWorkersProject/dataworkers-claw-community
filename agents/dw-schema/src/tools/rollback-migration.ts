...
export const rollbackMigrationHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('rollback_migration')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'rollback_migration' }) }],
      isError: true,
    };
  }
