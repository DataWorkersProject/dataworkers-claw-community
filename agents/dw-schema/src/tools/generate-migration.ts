...
export const generateMigrationHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('generate_migration')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'generate_migration' }) }],
      isError: true,
    };
  }
