...
export const registerKafkaSchemaHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('register_kafka_schema')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'register_kafka_schema' }) }],
      isError: true,
    };
  }
