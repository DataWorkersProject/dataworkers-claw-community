...
export const getConnectorStatusHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('get_connector_status')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'get_connector_status' }) }],
      isError: true,
    };
  }
