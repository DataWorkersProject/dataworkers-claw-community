...
export const createNessieBranchHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('create_nessie_branch')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'create_nessie_branch' }) }],
      isError: true,
    };
  }
