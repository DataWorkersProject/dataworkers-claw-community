...
export const emitLineageEventHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('emit_lineage_event')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'emit_lineage_event' }) }],
      isError: true,
    };
  }
