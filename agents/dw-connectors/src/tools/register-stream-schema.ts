...
export const registerStreamSchemaHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('register_stream_schema')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'register_stream_schema' }) }],
      isError: true,
    };
  }
