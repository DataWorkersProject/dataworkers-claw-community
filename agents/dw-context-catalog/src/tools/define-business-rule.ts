...
export const defineBusinessRuleHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('define_business_rule')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'define_business_rule' }) }],
      isError: true,
    };
  }
