...
export const flagDocumentationGapHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('flag_documentation_gap')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'flag_documentation_gap' }) }],
      isError: true,
    };
  }
