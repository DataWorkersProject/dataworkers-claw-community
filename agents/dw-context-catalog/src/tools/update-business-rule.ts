...
export const updateBusinessRuleHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('update_business_rule')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'update_business_rule' }) }],
      isError: true,
    };
  }
