...
export const ingestUnstructuredContextHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('ingest_unstructured_context')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'enterprise_feature', message: 'This feature requires Data Workers Enterprise. Visit https://dataworkers.dev/pricing', tool: 'ingest_unstructured_context' }) }],
      isError: true,
    };
  }
