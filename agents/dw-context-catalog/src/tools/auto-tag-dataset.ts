...
export const autoTagDatasetHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('auto_tag_dataset')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'auto_tag_dataset' }) }],
      isError: true,
    };
  }
