...
export const correctResponseHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('correct_response')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'correct_response' }) }],
      isError: true,
    };
  }
