...
export const flagStaleContextHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('flag_stale_context')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'flag_stale_context' }) }],
      isError: true,
    };
  }
