...
export const generateDocumentationHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('generate_documentation')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'generate_documentation' }) }],
      isError: true,
    };
  }
