...
export const runDataStewardHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('run_data_steward')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'enterprise_feature', message: 'This feature requires Data Workers Enterprise. Visit https://dataworkers.io/pricing', tool: 'run_data_steward' }) }],
      isError: true,
    };
  }
