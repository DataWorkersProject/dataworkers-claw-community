...
export const importTribalKnowledgeHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('import_tribal_knowledge')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'import_tribal_knowledge' }) }],
      isError: true,
    };
  }
