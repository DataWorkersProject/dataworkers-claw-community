...
export const revokeAuthorityHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('revoke_authority')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'revoke_authority' }) }],
      isError: true,
    };
  }
