...
export const markAuthoritativeHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('mark_authoritative')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'mark_authoritative' }) }],
      isError: true,
    };
  }
