...
export const updateLineageHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('update_lineage')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'update_lineage' }) }],
      isError: true,
    };
  }
