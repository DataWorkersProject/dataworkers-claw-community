...
export const enforceRbacHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('enforce_rbac')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.dev/pricing', tool: 'enforce_rbac' }) }],
      isError: true,
    };
  }
