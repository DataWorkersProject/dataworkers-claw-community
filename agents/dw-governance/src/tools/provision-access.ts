...
export const provisionAccessHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('provision_access')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'provision_access' }) }],
      isError: true,
    };
  }
