...
export const setSLAHandler: ToolHandler = async (args) => {
  if (!isToolAllowed('set_sla')) {
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'pro_feature', message: 'This feature requires Data Workers Pro. Visit https://dataworkers.io/pricing', tool: 'set_sla' }) }],
      isError: true,
    };
  }
